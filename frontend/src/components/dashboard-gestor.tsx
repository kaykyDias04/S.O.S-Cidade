...
const CRITICAL_TYPES = ["animal de grande porte solto", "foco de dengue / água parada", "vazamento de água potável", "buraco na via"];
...
export function DashboardGestor() {
  ...
  const renderTrend = (current: number, prev: number, invertGood = false, isPercentage = false) => {
    ...
    const percent = isPercentage ? diff : (Math.abs(diff) / prev) * 100;
